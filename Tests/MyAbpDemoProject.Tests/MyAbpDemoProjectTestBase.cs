...
using MyAbpDemoProject.Migrations.SeedData;
...
{
    public abstract class MyAbpDemoProjectTestBase : AbpIntegratedTestBase<MyAbpDemoProjectTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MyAbpDemoProjectTestBase()
        {
        ...
        protected void UsingDbContext(Action<MyAbpDemoProjectDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyAbpDemoProjectDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MyAbpDemoProjectDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MyAbpDemoProjectDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyAbpDemoProjectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyAbpDemoProjectDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyAbpDemoProjectDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyAbpDemoProjectDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MyAbpDemoProjectDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MyAbpDemoProjectDbContext, Task<T>> func)
        {
            T result;
